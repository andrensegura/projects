...
def get_games_list():
    print "<h2>Add games to %s's library:</h2>" % (username)
...
#                print "%s | %s <br>""" % (repr(game.lower()), repr(app[0].lower()))
#                continue
...
def add_games(amt_games, user):
    ...
    games_list = mysql.execute_mysql("""SELECT added_games FROM users WHERE username = %s"""
                   , (user,))[0][0]
    to_add = []
    ...
    mysql.execute_mysql("""UPDATE users SET added_games = %s WHERE username = %s;"""
                        , (str(to_add), user,) )
    print "success!"
